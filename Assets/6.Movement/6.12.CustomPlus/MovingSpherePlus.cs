...
namespace CustomPlus {
	public class MovingSpherePlus : MonoBehaviour {
		...
		[SerializeField, Range(0f, 10f)] private float jumpHeight = 3f, airJumpHeight = 2f, divingJumpHeight = 1f;
		[SerializeField, Range(0f, 5f)] private float jumpEarlyEndGravityScale = 3f, airJumpEarlyEndGravityScale = 1f;	// 小跳上升时重力变化
		[SerializeField, Range(0, 5)] private int maxAirJumps;
		[SerializeField, Range(0, 1)] private float steepJumpInputModifier = 0.7F;
		...
		[SerializeField] private bool freeDiving;
		[SerializeField] private bool divingClimbable;
		...
		[SerializeField, Min(0f)] private float driftAlignMaxSpeed = 100f, driftAlignAcceleration = 15f;
		[SerializeField, Range(0.01f, 1f)] private float driftThreshold = 0.3f, diveThreshold = 0.95f;
		[SerializeField, Range(2f, 100f)] private float maxDropSpeed = 30F, maxSteepDropSpeed = 3F;	// 最大掉落速度，最大擦墙掉落速度
		[SerializeField] private LayerMask probeMask = -1, stairsMask = -1, climbMask = -1, waterMask = 0;
		[SerializeField] private Material normalMaterial, climbingMaterial, driftingMaterial, divingMaterial;
